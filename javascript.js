...
document.getElementById("btn-again").onclick = showReadyScreen;
...
// ── Difficulty Settings ──
const DIFF_SETTINGS = {
  easy:   { cpuSpeed: 2.4, cpuReact: 0.32, errorY: 55, mistakeChance: 0.065, mistakeDur: 70, footerText: 'EASY MODE' },
  medium: { cpuSpeed: 4.6, cpuReact: 0.62, errorY: 26, mistakeChance: 0.018, mistakeDur: 42, footerText: 'PREPARE TO LOSE' },
  hard:   { cpuSpeed: 7.8, cpuReact: 0.92, errorY: 8,  mistakeChance: 0.005, mistakeDur: 18, footerText: 'GOOD LUCK...' },
};
...
function setDiff(diff, btn) {
  ...
  // Update CPU footer label
  const footer = document.getElementById('cpu-footer');
  if (footer) footer.textContent = DIFF_SETTINGS[diff].footerText;
}

function showReadyScreen() {
  state = "title";
  gameoverEl.classList.remove('on');
  if (readyEl) readyEl.classList.add('on');
}

// CPU difficulty tuning — resolved dynamically from currentDiff
function getCPUSpeed()        { return DIFF_SETTINGS[currentDiff].cpuSpeed; }
function getCPUReact()        { return DIFF_SETTINGS[currentDiff].cpuReact; }
function getCPUErrorY()       { return DIFF_SETTINGS[currentDiff].errorY; }
function getCPUMistakeChance(){ return DIFF_SETTINGS[currentDiff].mistakeChance; }
function getCPUMistakeDur()   { return DIFF_SETTINGS[currentDiff].mistakeDur; }

// Legacy constants kept for reference but overridden by difficulty functions above
const CPU_SPEED = 4.6;
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.code === "Space" && state === "over") showReadyScreen();
});
...
function startGame() {
  ...
  particles.length = 0;
  updateStatDOM();
  // Update CPU footer based on difficulty
  const footer = document.getElementById('cpu-footer');
  if (footer) footer.textContent = DIFF_SETTINGS[currentDiff].footerText;
}
...
function updateCPU(ts = 1) {
  ...
  const cpuMistakeChance = getCPUMistakeChance();
  const cpuMistakeDur    = getCPUMistakeDur();
  const cpuErrorY        = getCPUErrorY();
  const cpuReact         = getCPUReact();
  const cpuSpd           = getCPUSpeed();

  if (
    Math.random() < cpuMistakeChance &&
    cpu.mistakeTimer === 0 &&
    puck.vx > 0
  ) {
    cpu.mistakeTimer = cpuMistakeDur;
    cpu.errorY = (Math.random() - 0.5) * cpuErrorY * 2;
  }
  ...
  if (cornered || (farFromHome && !puckHeadingToMe)) {
    ...
    tx = clamp(puck.x + puck.vx * frames * cpuReact, minX, maxX);
    ty = clamp(puck.y + puck.vy * frames * cpuReact + err, minY, maxY);
  ...
  if (dist > 0.1) {
    const step = Math.min(dist, cpuSpd * ts);
    cpu.x += (dx / dist) * step;
    cpu.y += (dy / dist) * step;
  }
...
// Show ready screen on first load; game starts only after user clicks START
if (readyEl) {
  readyEl.classList.add('on');
  // Set data-diff attributes on buttons for color styling
  document.querySelectorAll('.btn-diff').forEach(btn => {
    const diff = btn.id.replace('btn-', '');
    btn.setAttribute('data-diff', diff);
  });
}
// Start render loop (canvas background). Mark running=true so startGame()
// won't spawn a second animation loop — it only resets state.
running = true;
loop();
